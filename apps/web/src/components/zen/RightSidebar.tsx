...
import ConceptMapViewer from "@/components/ObsidianGraph";
...
export default function RightSidebar({
  ...
  const renderToolContent = () => {
    ...
    switch (rightTab) {
          ...
          <div className="flex flex-col h-full">
            ...
            <div className="flex-1 overflow-y-auto">
              <ConceptMapViewer
                ...
                onNodeClick={(info) => {
                  if (info.startPage) {
                    setHighlightPage(info.startPage);
      ...
      case "quiz":
        ...
        return (
          <div className="p-3 space-y-3">
            {quizData.questions?.filter((q: QuizQuestion) => q.question && q.options && q.options.length > 0).map((q: QuizQuestion, i: number) => {
                  ...
                  <div className="space-y-0.5">
                    {q.options?.map((opt: string, j: number) => {
                      ...
                      return (
                        ...
                        >
                          {letter}) {opt}
                          {showCorrect && <span className="ml-1">✓</span>}
                          {showWrong && <span className="ml-1">✗</span>}
                        </button>
                      );
                    })}
                  </div>
      ...
      case "diagram":
        return <DiagramTab documentId={activeDoc.id} />;
