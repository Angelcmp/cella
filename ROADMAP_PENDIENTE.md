...
- [x] `/zen` composición final de 3 paneles (17/09/2026): anchos rebalanceados (288/440/56), headers uniformes `h-12` (título + acción) en los 3 paneles, `ChatInput` de barra única con botón teal, y tarjetas neutras con acento teal (citas + tool-cards). Referencia visual tipo Claude/Anthropic; sin cambios de función. `typecheck`/`build`/E2E 4/4 + captura ✅
